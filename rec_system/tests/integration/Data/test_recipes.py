...
from src.data.recipes import get_all_recipes, get_filtered_recipes, add_taste_profiles

API_KEY = '947992fa499844bb9e8e4a026e65a037'


class TestRecipes:
    @pytest.mark.parametrize("expected", [sorted(list(map(int, pd.read_csv('./src/data/ids.csv'))))])
    ...
    @pytest.mark.parametrize("expected", [pd.read_csv('./src/data/recipes.csv')])
