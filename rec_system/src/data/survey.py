...
COLUMNS_TO_DROP = ['W skali od 1 do 10 jak bardzo lubisz słone jedzenie', 'W skali od 1 do 10 jak bardzo lubisz słodkie jedzenie',
...
def convert_to_implicit_dataset():
    modified_dict = dict()
    df = pd.read_csv('./data/survey.csv')
    df.dropna()
    df = df.drop(COLUMNS_TO_DROP, axis=1)
    ...
    return pd.DataFrame.from_dict(modified_dict).T
